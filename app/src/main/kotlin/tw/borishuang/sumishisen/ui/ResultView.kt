    ...
    private fun setOnClickListener() {
        binding.btnAuto.setOnClickListener {
            if (PermissionUtil.isAccessibilityServiceEnabled(context)) {
                context.startAction(NavManager.ACTION_START_PERFORM_GESTURE)
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    delay(1000)
                    context.startAction(SumiService.ACTION_START)
                }
            }
            else {
        ...
        binding.btnAuto.setOnLongClickListener {
            context.startActivity(PermissionUtil.getAccessibilityIntent().addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
